...
def test_adding_overlapping_deptypes_to_different_versions_stays_parallel(
    ...
    """c1 and c2 are different concrete versions of one package, so neither edge implies the other
    however their depflags overlap, and both stay as parallel edges to pkg-b."""
...
def test_copy_preserves_parallel_edges_with_identical_attributes(mock_packages):
    """Two parallel edges to one name that agree on every attribute implies() compares, and differ
    only in what their targets depend on, are a legitimate state. A copy reproduces both."""
    ...
    # built without add_dependency_edge, so this does not depend on what implies() compares today
